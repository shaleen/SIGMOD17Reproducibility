...
def disagreementCountRandom(data=None, no_of_queries=None, support_count=None):
    if no_of_queries == None:
        no_of_queries = dbutils.DBUtils.no_of_queries()
    if support_count == None:
        support_count = dbutils.DBUtils.no_of_elements_in_support_set()
    if data == None:
        data = Utils.PricerUtils.disagreementMatrixRandom(dbutils.DBUtils.cursor, no_of_queries, support_count)
    y_plot = []
    for i in range(1, no_of_queries + 1):
        count = 0
        for j in range(0, support_count):
...
def disagreementEntropyCountRandom(data=None, no_of_queries=None, support_count=None):
    if no_of_queries == None:
        no_of_queries = dbutils.DBUtils.no_of_queries()
    if support_count == None:
        support_count = dbutils.DBUtils.no_of_elements_in_support_set()
...
def bucketEntropyRandom():
    no_of_queries = dbutils.DBUtils.no_of_queries()
    support_count = dbutils.DBUtils.no_of_elements_in_support_set()
    y_plot = []
    for i in range(1, no_of_queries+1):
        dict = {}
        dbutils.DBUtils.cursor.execute('select checksum_support, checksum_base from QueriesRandom where idQueries = '+str(i))
        res = dbutils.DBUtils.cursor.fetchall()
        for r in res:
            ret_str = r[0][1:]
            support_size = ret_str.split(';')
