...
class PricerUtils:
    ...
    @staticmethod
    def agreementMatrixRandom(cursor, no_of_queries, support_count):
        data = [[0 for i in range(no_of_queries+1)] for j in range(support_count+1)]
        for i in range(1, no_of_queries + 1):
            cursor.execute('select checksum_support, checksum_base from QueriesRandom where idQueries = ' + str(i))
            res = cursor.fetchall()
            checksum_base = res[0][1]
            for r in res:
                ret_str = r[0][1:]
    ...
    @staticmethod
    def disagreementMatrixRandom(cursor, no_of_queries, support_count):
        data = [[0 for i in range(no_of_queries+1)] for j in range(support_count+1)]
        for i in range(1, no_of_queries + 1):
            cursor.execute('select checksum_support, checksum_base from QueriesRandom where idQueries = ' + str(i))
            res = cursor.fetchall()
