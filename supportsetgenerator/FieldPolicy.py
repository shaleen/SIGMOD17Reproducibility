...
class Policy:
    ...
    def cellRandomOutsideDomain(self, table_name, field_name, count, counter):
        ...
        for i in range(0, count):
            ret_res = True
            while (ret_res):
                index_1 = random.randint(0, len(table_country.pk_values[table_name]) - 1)
                val = table_country.pk_values[table_name][index_1]
                string_1 = 'UPDATE ' + table_name + '_view' + ' SET ' + field_name + ' = ' + \
                           table_country.in_domain[table_name][field_name]()
            ...
            support_set.append([string_1, counter])
    ...
    def cellRandomWithinDomain(self, table_name, field_name, count, counter, newtablenames):
        ...
        for i in range(0, count):
            ...
            while (ret_res):
                ...
                index_1 = getattr(table_country, table_country.field_choosing_policy)(table_name)
                val = table_country.pk_values[table_name][index_1]
                where_cond = self.getWhereCondition(table_name, val)
                query_for_actual_1 = 'select ' + field_name + ' from ' + undotb + ' where ' + where_cond
                res_1 = self.executeQuery(query_for_actual_1)
                res_1 = table_country.in_domain[table_name][field_name](res_1)
                res_random = table_country.in_domain[table_name][field_name]()
            ...
            support_set_undo.append([string_1_undo, counter])
    ...
    def quotedString(self, table_name, value, index):
        if table_country.pk_type[table_name][index] == 'String':
            return "'"+str(value)+"'"
        return str(value)

    def getWhereCondition(self, table_name, val):
        ret_str = table_country.pk_names[table_name][0] + ' = ' + self.quotedString(table_name, val[0], 0)
        for i in range(1, len(val)):
            ret_str = ret_str +  ' AND ' + table_country.pk_names[table_name][i] + ' = ' + self.quotedString(table_name, val[i], i)
        return ret_str
    ...
    def swapField(self, table_name, field_name, count, counter, newtablesnames):
        if table_country.multiple_conditions_for_swap == False:
    ...
    def swapFieldSingleSet(self, table_name, field_name, count, counter, newtablenames=False):
        ...
        for i in range(0, count):
            ...
            while(ret_res):
                index_1 = getattr(table_country, table_country.field_choosing_policy)(table_name)
                index_2 = getattr(table_country, table_country.field_choosing_policy)(table_name)
                val_1 = table_country.pk_values[table_name][index_1]
                val_2 = table_country.pk_values[table_name][index_2]
                if index_1 == index_2:
                    continue
                ...
                query_for_actual_1 = 'select ' + field_name + ' from ' + undotb + ' where ' + where_cond_1
                #print query_for_actual_1
                res_1 = self.executeQuery(query_for_actual_1)
                query_for_actual_2 = 'select ' + field_name + ' from ' + undotb + ' where ' + where_cond_2
                ...
                if res_1 == res_2:
                    ...
                    ret_res = False
                str_1 = table_country.in_domain[table_name][field_name](res_1)
                str_2 = table_country.in_domain[table_name][field_name](res_2)
            ...
            support_set.append([string_1, counter])
            support_set.append([string_2, counter])
            support_set_undo.append(([string_1_undo, counter]))
            support_set_undo.append(([string_2_undo, counter]))
    ...
    def swapFieldMultipleSet(self, table_name, field_name, count, counter):
        ...
        support_set_undo = []
        support_set_undo_value = []
        randint = random.randint(1, len(table_country.support_fields[table_name]))
        fields = random.sample(range(0, len(table_country.support_fields[table_name])), randint)
        print fields
        ...
        for i in range(0, count):
            while(True):
                ...
                for j in range(0, len(fields)):
                    change_index = False
                    field_name = table_country.support_fields[table_name][fields[j]]
                    while (True):
